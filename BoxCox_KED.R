#########################################################################################################################
#####            This scripts Merges BoxCox transformed rain gauge with BoxCox transformedradar data            #########
#########################################################################################################################
...
# Variable folder
Folder <- "/home/fc14509/UK/P3/Transformed/"
...
# Box-Cox parameter values
l <- 0.1

# Result folder
Results <- "/home/fc14509/UK/P3/Results/BoxCox/"
...
RGfile <- paste0(Folder, "RG_BoxCox_", toString(l))
load(RGfile)
load(RGcoordfile)
RGcoord <- coord
...
for (m in 1:12) {
  ...
  radfile <- paste0(Folder, "Rad_2009",sprintf("%02d", m),"_BoxCox_", toString(l))
  load(radfile)
  ked_bc <- R
  Raddates <- R[,1:6]
  R <- R[,7:dim(R)[2]]
  ...
  for (t in 1:t_steps){
    ...
    if (checkzeros==bc(0,l)) { #Case in which both the radar and the gauges say it didn't rain: pred = 0, var = NA
      ked <- pred_grid
      names(ked) <- c("pred")
      ked$pred <- ked$pred*0
    } else if (!(rgcheckzeros==bc(0,l)) & radcheckzeros==bc(0,l)) {  # case in which it is impossible to do UK and we do OK. Variogram is calculated on rg
  ...
  save(ked, file=paste0(Results, "ked_2009",sprintf("%02d", m),"_BoxCox_", toString(l)))
}
